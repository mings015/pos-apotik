<script lang="ts">
  ...
  function buildUrl(extra: Record<string, string>) {
    ...
    return `/reports/cashiers?${new URLSearchParams({ ...f, ...extra })}`
  }

  async function exportExcel() {
    const XLSX = await import('xlsx')
    const headers = ['Nama Kasir', 'Total Transaksi', 'Total Pendapatan', 'Rata-rata / Transaksi']
    const dataRows = rows.map((r: any) => [
      r.name, r.transactions, r.revenue, r.avgTransaction,
    ])
    const ws = XLSX.utils.aoa_to_sheet([headers, ...dataRows])
    ws['!cols'] = [25, 15, 20, 20].map(w => ({ wch: w }))
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Kasir')
    XLSX.writeFile(wb, `laporan-kasir-${data.filters.dateFrom ?? 'semua'}-${data.filters.dateTo ?? ''}.xlsx`)
  }
</script>

<svelte:head><title>Laporan Kasir — PharmaPOS</title></svelte:head>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    ...
    <table class="w-full text-sm">
      <thead class="bg-gray-50 border-b border-gray-200">
        <tr>
          ...
          <th class="text-right px-4 py-3 text-xs font-semibold text-gray-600">Transaksi</th>
          <th class="text-right px-4 py-3 text-xs font-semibold text-gray-600">Total Pendapatan</th>
          <th class="text-right px-4 py-3 text-xs font-semibold text-gray-600">Rata-rata / Transaksi</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-gray-100">
        {#each rows as r}
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right text-gray-700">{r.transactions}</td>
            <td class="px-4 py-3 text-right font-semibold text-sky-600">{rp(r.revenue)}</td>
            <td class="px-4 py-3 text-right text-gray-600">{rp(r.avgTransaction)}</td>
          </tr>
        {:else}
          <tr><td colspan="4" class="px-4 py-10 text-center text-gray-400">Tidak ada data untuk filter ini</td></tr>
        {/each}
      </tbody>
    </table>
  </div>
  {#if meta.totalPages > 1}
    <div class="flex items-center justify-between px-4 py-3 border-t border-gray-100">
      <p class="text-xs text-gray-500">Total {meta.total} kasir</p>
